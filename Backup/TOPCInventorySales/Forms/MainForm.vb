...
#Region "Procedure"
    Sub AddMenu()
        ...
        Dim comm As New SqlCommand("GetUserMainMenu '" & CurrUser.USER_ID & "','" & isModule & "'", Conn)
    ...
    Sub AddSub(ByVal ParentMenuID As Integer, ByVal sender As Object)
        ...
        Dim Conn As New SqlConnection(cnString)
        Dim comm As New SqlCommand("GetUserSubMenu '" & isModule & "', '" & ParentMenuID & "','" & CurrUser.USER_ID & "'", Conn)
    ...
    Sub LoadForm()
        ...
        Try
            ...
            Me.Text = LoginForm.Text & " " & Version
    ...
    Sub ClickGOTO()
        If gotoX = 0 Then
            ...
            AddMenu()
            Call HideCotrols()
            Me.Text = "TOPC - Sales System" & " " & Version
            gotoX = 1
            tsGoto.ToolTipText = "Goto Warehouse"
            isSale = True
        ElseIf gotoX = 1 Then
            Me.mnMenu.Dispose()
            isModule = "Warehouse"
            AddMenu()
            Call HideCotrols()
            Me.Text = "TOPC - Warehouse System" & " " & Version
            gotoX = 0
            tsGoto.ToolTipText = "Goto Sales"
            isSale = False
        End If
    End Sub

    Private Sub HideCotrols()
        If CurrUser.User_Department = "P&MCD" Then
            tsNotify.Visible = True
            sepGoto.Visible = True
        Else
            tsNotify.Visible = False
            sepGoto.Visible = False
        End If
    End Sub
...
#Region "GUI"
    ...
    Private Sub tsGoto_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsGoto.Click
        Call ClickGOTO()

    End Sub

    Private Sub tsNotify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsNotify.Click
        Call popup("Promt")
    End Sub
