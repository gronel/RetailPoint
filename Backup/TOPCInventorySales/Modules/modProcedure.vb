...
Module modProcedure
    ...
    ''' <summary>
    ''' Update all Item in Transaction
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub UpdateTransactionStatus()
        Try
            _OpenTransaction()

            Using com As New SqlCommand("sp_UpdateStatus_Per_Trans'" & "PO" & "','" & isUndelivered & "'", _Connection, _Transaction)
                com.CommandType = CommandType.Text
                com.ExecuteNonQuery()
            End Using

            Using com1 As New SqlCommand("sp_UpdateStatus_Per_Trans'" & "JO" & "','" & isUndone & "'", _Connection, _Transaction)
                com1.CommandType = CommandType.Text
                com1.ExecuteNonQuery()
            End Using

            Using com1 As New SqlCommand("sp_UpdateStatus_Per_Trans'" & "PR-RR" & "','" & isNotPurchased & "','" & isPending & "'", _Connection, _Transaction)
                com1.CommandType = CommandType.Text
                com1.ExecuteNonQuery()
            End Using

            Using com1 As New SqlCommand("sp_UpdateStatus_Per_Trans'" & "PR-PO" & "','" & isUndelivered & "','" & isPlaced & "'", _Connection, _Transaction)
                com1.CommandType = CommandType.Text
                com1.ExecuteNonQuery()
            End Using

            Using com1 As New SqlCommand("sp_UpdateStatus_Per_Trans'" & "JR-RR" & "','" & isNotPurchased & "','" & isPending & "'", _Connection, _Transaction)
                com1.CommandType = CommandType.Text
                com1.ExecuteNonQuery()
            End Using

            Using com1 As New SqlCommand("sp_UpdateStatus_Per_Trans'" & "JR-JO" & "','" & isUndone & "','" & isPlaced & "'", _Connection, _Transaction)
                com1.CommandType = CommandType.Text
                com1.ExecuteNonQuery()
            End Using
    ...
    ''' <summary>
    '''Pop-up after login the system and if the transaction is inventory
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub popup(ByVal str As String)
        Call UpdateTransactionStatus()
        Dim a, b, c, d, e, f, h, i As String
        a = DBLookUp("Get_POPUP'" & "PR" & "'", "PR No.")
        b = DBLookUp("Get_POPUP'" & "JR" & "'", "JR No.")
        c = DBLookUp("Get_POPUP'" & "RR-PR" & "'", "PR No.")
        d = DBLookUp("Get_POPUP'" & "RR-JR" & "'", "JR No.")
        e = DBLookUp("Get_POPUP'" & "PO" & "'", "PO No.")
        f = DBLookUp("Get_POPUP'" & "JO" & "'", "JONo")
        h = DBLookUp("Get_POPUP'" & "PRstatus" & "'", "PRNo")
        i = DBLookUp("Get_POPUP'" & "JRstatus" & "'", "JRNO")

        Select Case str
            Case "Promt"
                If a = "" And b = "" And c = "" And d = "" And e = "" And f = "" And h = "" And i = "" Then
                    '-->nothing
                Else
                    With frm_POPUP
                        .MdiParent = MainForm
            ...
            Case "check"
                If a = "" And b = "" And c = "" And d = "" And e = "" And f = "" And h = "" And i = "" Then
                    MainForm.tsNotify.Enabled = False
                Else
                    MainForm.tsNotify.Enabled = True
                End If
        End Select
    End Sub
    ''' <summary>
    ''' Tread in the first report
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub treadCry()
        ...
        srcCry.Load(Application.StartupPath & "\Reports\rpt_200_CANo.rpt")
        With srcCry
        ...
        End With
        'frm_200_ReportV.rpt_viewer.ReportSource = srcCry
    End Sub
